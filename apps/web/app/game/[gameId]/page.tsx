...
export default function GamePage({
  ...
  useEffect(() => {
    ...
    // Spectators: auto-create guest session if not authenticated
    if (isSpectating && !isReady) {
      const auth = useAuthStore.getState();
      if (!auth.token || !auth.playerId || !auth.playerSecret) {
        auth.initGuest("spectator").catch(() => {
          // If guest init fails, redirect home
          router.replace("/");
        });
        return;
      }
    ...
    if (!isReady || !token || !playerId || !playerSecret) {
      if (!isSpectating) {
    ...
    if (isSpectating) {
      // Spectate mode: connect without registration
      connectToGame(gameId, token, true);
    } else {
      // Player mode: register and connect
      queueRejoin();
  ...
  const showRestoreOverlay = !hasHydrated || !isReady || (!isSpectating && phase === "idle");
  const showSpectateWaiting = isSpectating && !gameState;
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-primary)]">
      ...
      {showSpectateWaiting && !showRestoreOverlay && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[var(--bg-primary)]">
          <div className="text-2xl font-bold text-[var(--text-warm)] mb-3 font-[family-name:var(--font-fredoka)]">
            Connecting to game...
          </div>
          ...
          <div className="flex gap-1.5">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="w-3 h-3 rounded-full bg-[var(--accent-green)] animate-pulse"
                style={{ animationDelay: `${i * 0.2}s` }}
              />
            ))}
          </div>
      ...
      {!isSpectating && <GameControls />}

      {/* Mobile joystick */}
      {!isSpectating && <MobileJoystick visible={showJoystick} />}

      {/* Lobby overlay */}
      {phase === "lobby" && !isSpectating && (
        <LobbyOverlay
          gameId={gameId}
          playerId={playerId || ""}
          onStart={handleStart}
      ...
      {/* Game over overlay */}
      {phase === "ended" && !isSpectating && <GameOverOverlay onPlayAgain={handlePlayAgain} />}
    </div>
  );
}
